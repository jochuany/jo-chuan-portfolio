
export const SelectedProjectItem = ({ href, title, category, year, photo, tool }) => {
    return (

        <a href={href} className="md:border-none transition-all duration-300 ease group">
            <div className="flex justify-start items-start transition-all duration-300 ease">

                <div className="mr-4">
                    <div className="h-[80px] w-[140px] md:h-[120px] md:w-[160px] lg:h-[120px] lg:w-[120px] overflow-hidden relative group">
                        <div style={{ backgroundImage: `url(${photo})` }} className="h-full w-full md:grayscale
                bg-cover bg-center bg-no-repeat transition-all duration-500 ease md:group-hover:scale-110 md:group-hover:grayscale-0"></div>
                    </div>
                </div>

                <div className="flex flex-col">
                    <div className="text-[14px] transition-all duration-300 ease mb-2 font-bold md:group-hover:text-brand-primary">
                        {title}
                    </div>
                    <div className="text-[12px] transition-all duration-300 ease mb-2 font-normal md:group-hover:text-brand-primary">
                        {category}／{year}
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {tool && tool.length > 0 && tool.map((item, index) => (
                            <div
                                key={index}
                                className="text-[12px] px-2 py-0.5 bg-brand-graybg inset-shadow-sm rounded-full transition-all duration-300 ease
                            md:group-hover:bg-brand-primaryLight md:group-hover:text-brand-bg"
                            >
                                {item}
                            </div>
                        ))}
                    </div>
                </div>

            </div>
        </a>
    );
};